   ...
   await this.refreshSchedule();
    ...
    const schedule: Schedule = await response.json();
				...
				update((s) => ({
					...s,
					schedule,
					solving: schedule.solverStatus !== null && schedule.solverStatus !== 'NOT_SOLVING'
				}));

				if (
					schedule.solverStatus !== null &&
					schedule.solverStatus !== 'NOT_SOLVING' &&
					!state.autoRefreshInterval
				) {
					this.startAutoRefresh();
				} else if (
					(schedule.solverStatus === null || schedule.solverStatus === 'NOT_SOLVING') &&
					state.autoRefreshInterval
				) {
     ...
     headers: {
      'Authorization': `Bearer ${state.authtoken}`
		...
		async analyzeSchedule(): Promise<ScoreAnalysis | null> {
			try {
				const state = getState();
				if (!state.schedule || !state.schedule.score || state.schedule.score.includes('init')) {
					return null;
				}
