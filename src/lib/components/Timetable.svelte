...
<div class="container mx-auto p-6 bg-gray-50 shadow-lg rounded-lg">
  ...
  <div class="flex justify-center gap-4 mb-6">
    {#each roles as role}
      <button
        class={`px-4 py-2 font-medium rounded transition ${
          selectedRole === role ? 'bg-red-600 text-white' : 'bg-gray-200 hover:bg-gray-300'
        }`}
        ...
        <div class="overflow-auto">
          <div class="grid grid-cols-8 gap-0 border border-gray-300 rounded-lg">
            ...
            {#each hours as hour}
              ...
              {#each days as day}
                <div
                  class={`p-2 h-20 border border-gray-300 text-gray-700 flex items-center justify-center ${
                    scheduleData.lessons.some(
                      (lesson) =>
                        lesson.timeslot === hour && scheduleData?.timeslots.some((slot) => slot.dayOfWeek === day)
                    )
                      ? 'bg-blue-50'
                      : ''
                  }`}
                >
                  <!-- Docelowo wyświetl zajęcia -->
                </div>
              {/each}
            {/each}
          </div>
