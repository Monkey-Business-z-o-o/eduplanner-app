<script lang="ts">
  ...
  export let schedule: {
    lessons: Array<{ timeslot: string | null; room: string | null; subject: string; teacher: string; studentGroup: string; id: string }>;
  };
  ...
  async function addLesson() {
    try {
      ...
      newLesson = { timeslot: null, room: null, subject: '', teacher: '', studentGroup: '', id: '' };
    } catch (err: unknown) {
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {#each schedule.lessons.filter(lesson => lesson.timeslot == null || lesson.room == null) as lesson}
          <div class="bg-gray-50 rounded p-2">
            <LessonColor {lesson} selectedRole={mapActiveTabToRole(activeTab)} onDelete={handleDelete} />
          </div>
        {/each}
      </div>
    </div>
...
<section class="mt-8">
  ...
  <form on:submit|preventDefault={addLesson}>
    <div class="mb-2">
      ...
      <input id="subject" type="text" bind:value={newLesson.subject} class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required />
    </div>
    <div class="mb-2">
      <label for="teacher" class="block text-sm font-medium text-gray-700">Teacher</label>
      <input id="teacher" type="text" bind:value={newLesson.teacher} class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required />
    </div>
    <div class="mb-2">
      <label for="studentGroup" class="block text-sm font-medium text-gray-700">Student Group</label>
      <input id="studentGroup" type="text" bind:value={newLesson.studentGroup} class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required />
    </div>
    <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Add Lesson</button>
  </form>
</section>
