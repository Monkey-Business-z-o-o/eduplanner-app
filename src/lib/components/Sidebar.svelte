...
<div class="flex min-h-screen">
	<div
		class="flex flex-col items-center w-full lg:w-48 h-full space-y-6 overflow-hidden bg-red-700 text-white lg:items-start lg:space-y-4 lg:p-4 transition-all duration-300"
	>
		...
		<a
			href="/"
			class="flex flex-col items-center justify-center w-full h-34 bg-white hover:bg-red-200 transition-all duration-300 rounded-lg px-4 py-4"
		>
			<img width="552" height="552" src="/logo-pjatk.png" alt="EduPlaner Logo" />
			<span
				class="mt-2 text-transparent bg-clip-text bg-[linear-gradient(to_right,theme(colors.indigo.400),theme(colors.indigo.100),theme(colors.sky.400),theme(colors.fuchsia.400),theme(colors.sky.400),theme(colors.indigo.100),theme(colors.indigo.400))] font-bold text-xl animate-gradient"
			>
				EduPlaner
			</span>
		</a>
		...
		<nav class={`w-full ${isMenuOpen ? 'block' : 'hidden'} lg:block`}>
			{#each menuItems as item, index}
				<div>
					<a
						href={item.link}
						class="flex flex-col items-center w-full py-4 px-6 rounded-md hover:bg-white hover:text-black lg:flex-row lg:justify-start lg:px-4 transition-all duration-300 hover:scale-110"
						...
						{#if item.subcategories.length > 0}
							...
							>
								<path
									...
									stroke-linejoin="round"
									stroke-width="2"
									d={activeCategory === index ? 'M19 9l-7 7-7-7' : 'M9 5l7 7-7 7'}
								/>
							</svg>
						{/if}
					</a>
						...
						<div class="flex flex-col items-center w-full mt-2 lg:items-start">
							{#each item.subcategories as subcategory}
								<a
									...
									class="flex flex-col items-center w-full py-2 px-4 rounded-md hover:bg-white hover:text-black lg:flex-row lg:justify-start lg:px-4 transition-all duration-300 hover:scale-110"
								>
