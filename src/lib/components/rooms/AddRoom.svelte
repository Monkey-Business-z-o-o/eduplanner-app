<script lang="ts">
  import SectionHeader from '../common/SectionHeader.svelte'; // Dodaj import
  let newRoomName: string = '';
  let responseMessage: string | null = null;
  let error: string | null = null;
  let isRoomMenuOpen: boolean = false; // Używamy do sterowania otwieraniem sekcji

  const addRoom = async () => {
    try {
      const response = await fetch('https://backend.kebson.fun/rooms', {
        method: 'POST',
        headers: {
          Accept: 'application/hal+json',
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ name: newRoomName }),
      });

      if (!response.ok) {
        throw new Error('Failed to add room');
      }

      responseMessage = 'Dodano pokój pomyślnie';
      newRoomName = ''; // Reset the field after adding the room
    } catch (err: unknown) {
      if (err instanceof Error) {
        error = err.message;
      }
    }
  };
</script>

<div class="max-w-md mt-10 p-6 bg-gray-300 rounded-lg shadow-md">
  <!-- Użycie SectionHeader -->
  <SectionHeader title="Dodaj nowy pokój" bind:isOpen={isRoomMenuOpen} />

  {#if isRoomMenuOpen}
    <div class="bg-red-50 p-4 rounded-lg shadow-sm mt-4">
      <form on:submit|preventDefault={addRoom} class="space-y-4">
        <div>
          <label for="roomName" class="block text-sm font-medium text-gray-700">Nazwa pokoju</label>
          <input
            type="text"
            id="roomName"
            bind:value={newRoomName}
            required
            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 sm:text-sm"
          />
        </div>
        <button
          type="submit"
          class="w-full py-2 px-4 bg-red-800 text-white font-semibold rounded-md shadow hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
        >
          Dodaj pokój
        </button>
      </form>
    </div>
  {/if}

  {#if responseMessage}
    <p class="mt-4 text-green-600">{responseMessage}</p>
  {/if}

  {#if error}
    <p class="mt-4 text-red-600">Błąd: {error}</p>
  {/if}
</div>
