  ...
  const menuItems = [
    {
      ...
      subcategories: [
        ...
        { name: 'View by rooms', link: '/schedule/room' }
      ]
    },
    {
      name: 'Messages',
...
<div class="flex min-h-screen">
  <div class="flex flex-col items-center w-full lg:w-48 h-full space-y-6 overflow-hidden bg-red-700 text-white lg:items-start lg:space-y-4 lg:p-4 transition-all duration-300">
    ...
    <a href="/static" class="flex flex-col items-center justify-center w-full h-34 bg-white hover:bg-red-200 transition-all duration-300 rounded-lg px-4 py-4">
    ...
    <nav class={`w-full ${isMenuOpen ? 'block' : 'hidden'} lg:block`}>
      {#each menuItems as item, index}
        <div>
          <a href={item.link} class="flex flex-col items-center w-full py-4 px-6 rounded-md hover:bg-white hover:text-black lg:flex-row lg:justify-start lg:px-4 transition-all duration-300 hover:scale-110" on:click={() => { toggleCategory(index); selectSection(item.name); }}>
            <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
