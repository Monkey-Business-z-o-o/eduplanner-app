		...
		<main class="flex-1 container mx-auto px-4 py-8">
			...
			<div class="mb-6">
				<nav class="flex gap-4">
					{#each ['byRoom', 'byTeacher', 'byStudentGroup'] as tab}
						<button
							class="px-4 py-2 rounded transition-colors {activeTab === tab
			...
			{#if schedule}
				...
				<div class="bg-white rounded-lg shadow-lg p-6 mb-8">
					...
					<Lessons />
					<!-- Here we add the Lessons component -->
				</div>
				...
				<div class="bg-white rounded-lg shadow-lg p-6 mb-8">
					...
					<Room />
					<!-- Here we add the Room component -->
				</div>
			{/if}
		</main>
...
{#if showAnalysisModal}
	<ScoreAnalysisModal {schedule} onClose={() => (showAnalysisModal = false)} />
{/if}
